...
public class Game {
     ...
     */
    public Game(Hero hero, Monster monster, Monster BossMonster, Map Starting, Map Final) {
        this.hero = hero;
    ...
    public void actualGameLoop() throws IOException{
        ...
        while (hero.getLife() >= 0 && BossMonster.getLife() >= 0){
            ...
            System.out.println("Your Hero stats is");
            ...
            if (Current.getName().equalsIgnoreCase("Noob Area"))
                destination = Final; 
            else
                destination = Starting; 
            
            System.out.println("(4)     go to Noob Area/Boss Area: " + destination.getName() );
    ...
    public void playerAddXp(int xp){
        ...
        if (hero.getXp() > hero.getLevelUp() * hero.getLevelUp())
            hero.setLevel(hero.getLevelUp() + 1);
